...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        if random.random() <= self.getMaxBirthProb()*(1-popDensity):
            ...
            raise NoChildException("The Virus Particle Does Not Reproduce")
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    # TODO
    trialPops = np.zeros((numTrials, 300))
    meanPop = np.array(300)
    for trial in range(numTrials):
        ...
        for step in range(300):
            ...
            steps.append(step)
        trialPops[trial]=pops    
    #print(trialPops[99]) 
    meanPop = np.mean(trialPops, axis=0)
    print(meanPop)
    
    pylab.title('Mean Virus Populations ('
                + str(numTrials) + ' trials)')
    pylab.xlabel('Time Step')
    pylab.ylabel('Mean Virus Population')
    pylab.plot(steps, meanPop, color ='r', label = 'SimpleVirus')
    pylab.xlim(0, 300)
    pylab.legend()
    pylab.figure()
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        # TODO
        resistances_new = self.getResistances().copy()
          
        for key in self.getResistances().keys():
            if random.random() <= 1-self.getMutProb():
                resistances_new[key] = self.getResistances()[key]
            else:
                resistances_new[key] = not(self.getResistances()[key])
        if sum([self.isResistantTo(drug) for drug in activeDrugs])==len(activeDrugs) \
            and random.random() <= self.getMaxBirthProb()*(1-popDensity):
            return ResistantVirus(self.getMaxBirthProb(), self.getClearProb(), \
                resistances_new, self.getMutProb())
        else:
            raise NoChildException("The Virus Particle Does Not Reproduce")
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        resistPop = []
        for virus in self.getViruses():
            if sum([virus.isResistantTo(drug) for drug in drugResist])==len(drugResist):
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    pylab.xlabel('Time Step')
    pylab.ylabel('Mean Virus Population')
    pylab.plot(steps, meanPop, color ='r', label = 'ResistantVirus')
    pylab.xlim(0, 300)
    pylab.legend()
    pylab.figure()

resistances = {"guttagonol" : True, 'srinol' : False}
simulationWithDrug(100, 1000, 0.1, 0.05, resistances, 0.1, 100)